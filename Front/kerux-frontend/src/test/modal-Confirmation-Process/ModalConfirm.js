...
const ModalConfirmNet = (props) => {
  ...
  const confirmNettoyage = async (e) => {
    ...
    await NettoyageService.ajouterNettoyage(
      props.categorie,
      props.typeProd,
      etape,
      props.poids,
      ...
      for (var i = 0; i < props.agents.length; i++) {
        ...
        Service_AgentProcess.ajouterAgentProcess(
          res.data.id_gnerate,
          props.agents[i].id_personne
        ).then((result) => {
        ...
        <Modal.Body>
          <div class="list-group">
            ...
            <span className="list-group-item list-group-item-action list-group-item-light">
              {" "}
              <span className="attributs">Poids:</span> {props.poids / 1000} Kg
            </span>
