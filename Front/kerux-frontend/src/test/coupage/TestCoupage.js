...
console.log(currentPosts2);
  ...
  const getData = () => {
    ...
    serviceActuelProcess
      .getActualProcesssStockBlock("nettoyage")
      .then((res) => {
        if (res.data.length === 0) {
          setProduitbloquant(true);
          serviceActuelProcess
...
*/
  ...
  const plusId = (e) => {
    ...
    if (produitBloquant === false && produitBloquantStock === false) {
      serviceActuelProcess
        .getIdProcess("nettoyage", boxes[0].id_box)
        .then((res) => {
          console.log(res.data);
      ...
      serviceActuelProcess
        .getIdBloquant("nettoyage", boxes[0].id_box)
        .then((res) => {
          console.log(res.data);
          if (res.data === "boxe n'existe pas") {
            setMessage("Vérifier votre ID ");
  ...
  const confirmer = (e) => {
    ...
    if (boxes.length > 1) {
      for (var i = 0; i < boxCoupageTab.length; i++) {
        CoupageService.ajouterBoxCouper(
          boxCoupageTab[i].id_produit,
          boxCoupageTab[i].id_enregistrement,
          boxCoupageTab[i].id_nettoyage,
          boxCoupageTab[i].id_generate
        ).then((res) => {
          console.log(res.data);
        });
            ...
            {((produitBloquant === false && produitBloquantStock === false) ||
              ...
              currentPosts.map((p, key) => (
                <tr
                  ...
                  style={{
                    background: `${
                      dateModif(p.date_alert) <= dateToday()
                        ? "#E8C4C4"
                        : "white"
                    }`,
                  }}
                ...
                return (
                  <div className="input-group col-sm-10" key={key}>
                    <BoxCoupage
                      id_box={box.id_box}
                      k={key}
                      supprimerBox={supprimerBox}
                      onIdChange={(newId_box) => {
                        const newBoxes = [...boxes];
