...
const TestSortie = (props) => {
  ...
  const getData = () => {
    serviceActuelProcess
      .getActualProcessBlock("conditionnement")
      .then((res) => {
        ...
        if (res.data.length === 0) {
          ...
          serviceActuelProcess
            .getActualProcess("conditionnement")
            .then((res) => {
    ...
    serviceActuelProcess
      .getActualProcesssStockBlock("conditionnement")
      .then((res) => {
        if (res.data.length === 0) {
          serviceActuelProcess
            .getActualProcesssStock("conditionnement")
            .then((res) => {
  ...
  const getProcess = (e) => {
    ...
    if (produitBloquant === false && produitBloquantStock === false) {
      serviceActuelProcess.getIdProcess("conditionnement", id).then((res) => {
      ...
      serviceActuelProcess.getIdBloquant("conditionnement", id).then((res) => {
        if (res.data === "boxe n'existe pas") {
          setMessage("Vérifier votre ID");
          handleShow(true);
        } else if (res.data.fk_stock === null) {
