...
ajouterProcessCondit = (req, res) => {
  ...
  pool.query(
    queries.ajouterProcessCondHist,
    [
      categorie,
      nom_produit,
      etape,
...
modifierProcessCondit = (req, res) => {
  ...
  pool.query(
    queries.ModifyProcessCond,
    [fk_stock, id_conditionnement],
    (error, result) => {
      if (error) throw error;
...
getProcessId = (req, res) => {
  ...
  pool.query(queries.getProcessByIdCondit, [id], (error, result) => {
    const notExiste = result.rows.length;
    if (!notExiste) {
      res.send("ID n'existe pas");
    } else {
      pool.query(queries.getProcessbyIdCoupage, [id], (error, resultat) => {
...
getProcessByDateHeure = (req, res) => {
  pool.query(q.getProcessBydateHeure, ["conditionnement"], (error, result) => {
...
getProduitEnAttente = (req, res) => {
  pool.query(actual.ActualProcess, ["coupage"], (error, result) => {
    res.status(200).json(result.rows);
  });
};
getProduitEnStock = (req, res) => {
  pool.query(actual.ActualProcessEnStock, ["coupage"], (error, result) => {
    res.status(200).json(result.rows);
  });
};

getProcessCoupTble = (req, res) => {
  pool.query(actual.ProcessTble, ["coupage"], (error, result) => {
    res.status(200).json(result.rows);
  });
};

getProcessCondiTble = (req, res) => {
  pool.query(actual.ProcessTble, ["conditionnement"], (error, result) => {
    res.status(200).json(result.rows);
  });
};
