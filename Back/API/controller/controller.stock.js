...
ajouterStock = (req, res) => {
  ...
  pool.query(
    queries.ajouterStock,
    [date_entree, fk_entrepot],
    (error, result) => {
...
modifierStock = (req, res) => {
  ...
  pool.query(
    queries.ModifyStock,
    [date_sortie, poids_sortie, id_stock],
    (error, result) => {
      if (error) throw error;
...
getStock = (req, res) => {
  pool.query(q.getStock, (error, result) => {
    if (error) throw error;
...
getStockByDate = (req, res) => {
  ...
  pool.query(q.getStockByDate, [datee], (error, result) => {
    res.status(200).json(result.rows);
  });
};
...
getStockByEtape = (req, res) => {
  ...
  pool.query(q.getStockByEtape, [etape], (error, result) => {
    if (error) throw error;
    res.status(200).json(result.rows);
  });
};
