...
ajouterProcessSortie = (req, res) => {
  ...
  pool.query(
    queries.ajouterProcessSortieHist,
    [
      categorie,
      nom_produit,
      etape,
...
const supprimerProcess = (req, res) => {
  ...
  console.log(id);
  pool.query(queries.supprimerProcess, id_enregistrement, (error, result) => {
    if (error) throw error;
  });
};
...
getProcessId = (req, res) => {
  ...
  pool.query(queries.getProcessByIdSortie, [id], (error, result) => {
    ...
    if (!notExiste) {
      ...
      pool.query(
        queries.getProcessbyIdConitionnement,
        [id],
        (error, resultat) => {
...
getProcessByDateHeure = (req, res) => {
  pool.query(q.getProcessBydateHeure, ["sortie"], (error, result) => {
...
getProduitEnAttente = (req, res) => {
  pool.query(actual.ActualProcess, ["conditionnement"], (error, result) => {
    res.status(200).json(result.rows);
  });
};
...
getProcessSortieTble = (req, res) => {
  pool.query(actual.ProcessTble, ["sortie"], (error, result) => {
    res.status(200).json(result.rows);
  });
};
