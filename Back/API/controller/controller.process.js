...
ajouterBon = (req, res) => {
  ...
  pool.query(
    queries.ajouterReception,
    [fk_fournisseur, acheteur, type_bon, datee, heure, recepteur, livreur],
    (error, result) => {
...
ajouterProduitFourni = (req, res) => {
  var datee = new Date();
  var heure = HeureNow();
  const { categorie, nom_produit, poids_fourni, nombre_fourni, fk_bon } =
    req.body;
  pool.query(
    queries.ajouterProduitFourni,
    [categorie, nom_produit, poids_fourni, nombre_fourni, datee, heure, fk_bon],
    (error, result) => {
      console.log(result);
      if (error) throw error;
      res.status(200).json(result.rows);
...
modifierProcess = (req, res) => {
  ...
  pool.query(queries.ModifyProcess, [fk_stock, id_gnerate], (error, result) => {
    if (error) throw error;
    res.status(200).send("process bien modifier ");
  });
  pool.query(
    queries.ModifyProcessHist,
    [fk_stock, id_gnerate],
    (error, result) => {
      if (error) throw error;
      res.status(200);
...
getProcessId = (req, res) => {
  ...
  pool.query(queries.getProcessById, [id], (error, result) => {
    if (error) throw error;
...
getbonByFournisseur = (req, res) => {
  pool.query(queries.getbonByFournisseur, (error, result) => {
    if (error) throw error;
    res.status(200).json(result.rows);
  });
};
...
getBon_NomFournisseur = (req, res) => {
  ...
  pool.query(
    queries.getBonByNomFournisseur,
    [nom_fournisseur + "%"],
    (error, result) => {
      if (error) throw error;
      res.status(200).json(result.rows);
...
getProdFourni = (req, res) => {
  pool.query(queries.getProdFourni, (error, result) => {
    if (error) throw error;
    res.status(200).json(result.rows);
  });
};
...
getProdByNomFourniseur = (req, res) => {
  const nom_fournisseur = req.params.nom_fournisseur;
  pool.query(
    queries.getProdByNomFourniseur,
    [nom_fournisseur + "%"],
    (error, result) => {
      if (error) throw error;
      res.status(200).json(result.rows);
