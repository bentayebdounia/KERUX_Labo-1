...
getProduitEnAttente = (req, res) => {
  ...
  var date = new Date();
  pool.query(queries.ActualProcess, [etape, date], (error, result) => {
...
getProduitEnStock = (req, res) => {
  const etape = req.params.etape;
  var date = new Date();
  pool.query(queries.ActualProcessEnStock, [etape, date], (error, result) => {
    res.status(200).json(result.rows);
  });
};
...
getProduitEnAttenteBlock = (req, res) => {
  const etape = req.params.etape;
  var date = new Date();
  pool.query(queries.ActualProcessBlock, [etape, date], (error, result) => {
    res.status(200).json(result.rows);
  });
};
getProduitEnStockBlock = (req, res) => {
  ...
  var date = new Date();
  pool.query(
    queries.ActualProcessEnStockBlock,
    [etape, date],
    (error, result) => {
      res.status(200).json(result.rows);
    }
...
getIdProcess = (req, res) => {
  ...
  var date = new Date();
  pool.query(
    queries.getIdProcess,
    [etape, date, id_gnerate],
    (error, result) => {
